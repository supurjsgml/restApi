...
# =========================================================================
#  Spring Boot Nginx 무중단 배포 스크립트 (8080 <-> 9080)
# =========================================================================
...
# 현재 가동 중인 포트 확인 (8080 포트가 열려 있는지 체크)
CURRENT_PORT=8080
TARGET_PORT=9080

# 8080 포트가 활성화(LISTEN) 상태인지 확인
if lsof -i :8080 > /dev/null; then
    CURRENT_PORT=8080
    TARGET_PORT=9080
else
    # 8080이 닫혀 있으면 9080이 돌고 있거나 최초 기동임
    if lsof -i :9080 > /dev/null; then
        CURRENT_PORT=9080
        TARGET_PORT=8080
    else
        # 둘 다 안 열려 있는 최초 기동일 경우 기본 포트를 8080으로 잡음
        CURRENT_PORT=9080
        TARGET_PORT=8080
    fi
fi
...
# 새 서버 헬스체크 (정상 구동될 때까지 최대 30초 대기)
...
# Nginx 설정 스위칭 (8080 -> 9080 또는 9080 -> 8080)
